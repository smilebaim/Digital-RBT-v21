...
// Kecamatan names per kabupaten (seeded data)
...
const DESA_PER_KEC = ["Desa Induk", "Gampong Baru", "Gampong Tengah", "Alue Papeun", "Blang Dalam", "Cot Seunong"];

function generateBanjirData(kabupaten: string) {
  const kecamatanList = KECAMATAN_NAMES[kabupaten] || ["Kecamatan 1", "Kecamatan 2", "Kecamatan 3"];
  const result: object[] = [];

  kecamatanList.forEach((kec) => {
    const numDesa = rand(2, 6);
    const desaItems = DESA_PER_KEC.slice(0, numDesa).map((desa, di) => ({
      level: "desa",
      kabupaten,
      kecamatan: kec,
      desa: `${desa} ${di + 1}`,
    ...
    const kecSummary = desaItems.reduce(
      ...
      {
        level: "kecamatan", kabupaten, kecamatan: kec, desa: null,
        terdampak_kk: 0, terdampak_jiwa: 0,
        pengungsi_kk: 0, pengungsi_jiwa: 0,
        korban_hilang: 0, korban_meninggal: 0,
...
export async function GET(request: NextRequest) {
  ...
  // source param accepted but not used (dummy data)

  let data: object[] = [];

  if (kabupaten) {
    data = generateBanjirData(kabupaten);
  } else {
    // All kabupaten
    KABUPATEN.forEach((kab) => {
      data.push(...generateBanjirData(kab.nama));
    });
  }

  // Filter by kecamatan
  if (kecamatan) {
    data = data.filter((d: unknown) => {
  ...
  const totals = data
    .filter((d: unknown) => (d as Record<string, unknown>).level === "kecamatan")
    .reduce(
      (acc, d: unknown) => {
