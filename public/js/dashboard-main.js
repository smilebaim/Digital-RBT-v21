...
let pendingFetches = Promise.resolve();
async function fetchSupabaseEndpoint(endpoint, cacheKey, useLocalStorage = true, localStorageTTL = 'default') {
  ...
  pendingFetches = pendingFetches.then(exec).catch(exec);
...
function renderBantuanHistorisTable() {
  const tbody = document.getElementById('tableBantuanHistoris');
  if (!tbody) return;

  const data = state.data.banlog?.data || [];
  if (data.length === 0) {
    tbody.innerHTML = '<tr><td colspan="4" class="text-center p-4 text-gray-400">Tidak ada data historis</td></tr>';
    return;
  }
  ...
  const avgIKS = countIKS > 0 ? (sumIKS / countIKS) : 0;
  const avgIKE = countIKE > 0 ? (sumIKE / countIKE) : 0;
  const avgIKL = countIKL > 0 ? (sumIKL / countIKL) : 0;

  // Mock data for previous year
  const prevIKS = Math.max(0, avgIKS - 0.05);
  const prevIKE = Math.max(0, avgIKE - 0.02);
  const prevIKL = Math.max(0, avgIKL + 0.03);

  const dims = [
    { name: 'Ketahanan Sosial (IKS)', curr: avgIKS, prev: prevIKS },
    { name: 'Ketahanan Ekonomi (IKE)', curr: avgIKE, prev: prevIKE },
    { name: 'Ketahanan Lingkungan (IKL)', curr: avgIKL, prev: prevIKL }
  ];
  ...
  tbody.innerHTML = dims.map(dim => {
    ...
    let trendHtml = '';
    if (diff > 0) {
      trendHtml = `<span class="text-green-600 font-bold"><i class="fas fa-arrow-up mr-1"></i>+${diff.toFixed(2)}</span>`;
    } else if (diff < 0) {
      trendHtml = `<span class="text-red-600 font-bold"><i class="fas fa-arrow-down mr-1"></i>${diff.toFixed(2)}</span>`;
    } else {
      trendHtml = `<span class="text-gray-500 font-bold"><i class="fas fa-minus mr-1"></i>0.00</span>`;
    }

    return `
      <tr class="border-b hover:bg-gray-50">
        <td class="p-3 font-medium text-xs">${dim.name}</td>
        <td class="p-3 text-center text-gray-500">${dim.prev.toFixed(2)}</td>
        <td class="p-3 text-center font-bold">${dim.curr.toFixed(2)}</td>
 ...
 */
async function preloadOtherTabs() { return; 
